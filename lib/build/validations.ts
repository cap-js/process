...
export function validateAllowedAnnotations(allowedAnnotations: string[], def: CsnDefinition, entityName: string, annotationPrefix: string, buildPlugin: ProcessValidationPlugin) {
    for (const key of Object.keys(def)) {
      if (key.startsWith(annotationPrefix + '.') && !allowedAnnotations.includes(key)) {
        buildPlugin.pushMessage(WARNING_UNKNOWN_ANNOTATION(entityName, key, allowedAnnotations), WARNING)
      }
    }
}
...
export function validateOnAnnotation(def: any, entityName: string, annotationOn: string, buildPlugin: ProcessValidationPlugin) {
    if (typeof def[annotationOn] !== 'string') {
      ...
      if(!VALID_EVENTS.includes(def[annotationOn]) && !actions.includes(def[annotationOn])) {
        buildPlugin.pushMessage(ERROR_ANNOTATION_MUST_BE_EVENT_OR_ACTION(entityName, annotationOn), ERROR)
      }
  }
...
export function validateCascadeAnnotation(def: any, entityName: string, annotationCascade: string, buildPlugin: ProcessValidationPlugin) {
...
export function validateRequiredStartAnnotations(hasOn: boolean, hasId: boolean, entityName: string, buildPlugin: ProcessValidationPlugin) {
    if (hasOn && !hasId) {
        buildPlugin.pushMessage(ERROR_START_ON_REQUIRES_ID(entityName, PROCESS_START_ON, PROCESS_START_ID), ERROR)
    }
    if (hasId && !hasOn) {
        buildPlugin.pushMessage(ERROR_START_ID_REQUIRES_ON(entityName, PROCESS_START_ID, PROCESS_START_ON), ERROR)
    }

  }
  
  export function validateIdAnnotation(def: any, entityName: string, processDef: CsnDefinition | undefined, buildPlugin: ProcessValidationPlugin) {
  ...
  if(!processDef) {
      buildPlugin.pushMessage(WARNING_NO_PROCESS_DEFINITION(entityName, PROCESS_START_ID, def[PROCESS_START_ID]), WARNING)
  }
...
export function validateInputTypes(buildPlugin: ProcessValidationPlugin, entityName: string, def: CsnDefinition, processDef: CsnDefinition, allDefinitions: Record<string, CsnDefinition> | undefined) {
  ...
  const entityAttributes = getElementNamesAndTypes(buildPlugin,def as CsnEntity, allDefinitions || {}, new Set<string>());
  ...
  if(!processDefInputs['businesskey']) {
    buildPlugin.pushMessage(ERROR_START_BUSINESSKEY_INPUT_MISSING(entityName, def[PROCESS_START_ID]), ERROR);
  } else {
...
function validateElementTypeMatch(
  ...
  if (processType.isMandatory && !entityType.isMandatory) {
    buildPlugin.pushMessage(
      WARNING_MANDATORY_MISMATCH(entityName, key, processDefId),
      WARNING
    );
  }
